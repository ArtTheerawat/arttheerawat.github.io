...
def sha(p: Path) -> str:
    """Hash data.json ignoring volatile `generated_at` + `usage.updated_at`
    timestamps so we only commit+push when actual content changed, not on the
    per-run timestamp refresh."""
    if not p.exists():
        return ""
    raw = p.read_bytes()
    try:
